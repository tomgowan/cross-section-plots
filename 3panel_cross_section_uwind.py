...
#for i in range(0,np.min([ds_no.dbz[:,0,0,0].size, ds_small.dbz[:,0,0,0].size, ds_tall.dbz[:,0,0,0].size])):
    ...
    for j in range(1,4):
        ...
        #Levels for variable
        levels = np.arange(-10,10.01,0.5)
        levels_ticks = np.arange(-10,10.01,10)
        
        #Plot variable
        ref_plot = plt.contourf(x2d, y2d, model_run.u[i,zmin:zmax[j-1],ymid,xmin:xmax], levels, cmap = cmap_precip, extend = 'both', alpha = 1)
        ...
        #Labels
        sub_title = ['No Mountain/30ms Wind', '500m Mountain/30ms Wind', '1500m Mountain/30ms Wind']

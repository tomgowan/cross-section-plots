...
for i in range(0,np.min([ds_no.dbz[:,0,0,0].size, ds_small.dbz[:,0,0,0].size, ds_tall.dbz[:,0,0,0].size])):
    ...
    for j in range(1,4):
        ...
        #Levels for variable
        levels = np.arange(258,286.01,0.1)
        levels_ticks = np.arange(258,286.01,2)
        
        #Plot variable
        ref_plot = plt.contourf(x2d, y2d, model_run.th[i,zmin:zmax[j-1],ymid,xmin:xmax], levels, cmap = cmap_precip, extend = 'both', alpha = 1)
        ...
        if j == 1:
            #Title
            plt.title("Potential Temperature [elapsed time = %d seconds]"  % secs, fontsize = 22, y = 1.19) 
    ...
    plt.text(0.25, -0.14, 'K', fontsize = 19)
    
    
    path = '/uufs/chpc.utah.edu/common/home/u1013082/public_html/phd_plots/cm1/'
    plt.savefig('%s' % path + 'png_for_gifs/3panel_cross_section_30ms_th_%03d.png' % i, dpi = 100)
    plt.close(fig)
    

##Build GIF

os.system('convert -delay 12 -quality 100 /uufs/chpc.utah.edu/common/home/u1013082/public_html/phd_plots/cm1/png_for_gifs/3panel_cross_section_20ms_th_*.png /uufs/chpc.utah.edu/common/home/u1013082/public_html/phd_plots/cm1/gifs/3panel_cross_section_20ms_th.gif')

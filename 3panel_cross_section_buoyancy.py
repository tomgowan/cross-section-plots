...
#Read in with xarray
ds_no = xr.open_dataset('/uufs/chpc.utah.edu/common/home/steenburgh-group7/tom/cm1/cm1r19/run/output/cm1out_200m_30ms_0000m.nc')
ds_small = xr.open_dataset('/uufs/chpc.utah.edu/common/home/steenburgh-group7/tom/cm1/cm1r19/run/output/cm1out_200m_30ms_0500m.nc')
ds_tall = xr.open_dataset('/uufs/chpc.utah.edu/common/home/steenburgh-group7/tom/cm1/cm1r19/run/output/cm1out_200m_30ms_1500m.nc')
...
colors1 = np.array(nclcmaps.colors['WhiteBlueGreenYellowRed']) #'prcp_1' for precip
...
for i in range(0,np.min([ds_no.dbz[:,0,0,0].size, ds_small.dbz[:,0,0,0].size, ds_tall.dbz[:,0,0,0].size])):
    ...
    for j in range(1,4):
        ...
        plt.subplots_adjust(left=0.06, bottom=0.06, right=0.88, top=0.93, wspace=0.4, hspace=0)
        ...
        lmin = -0.2
        lmax =  0.2
        levels = np.arange(lmin,lmax+0.0001,(lmax-lmin)/200)
        levels_ticks = np.arange(lmin,lmax+0.0001,(lmax-lmin)/10)
        
        #Plot variable
        ref_plot = plt.contourf(x2d, y2d, model_run.buoyancy[i,zmin:zmax[j-1],ymid,xmin:xmax], levels, cmap = cm.bwr, extend = 'both', alpha = 1)
        ...
        #Labels
        sub_title = ['No Terrain/30$\mathregular{ms^{-1}}$', '500m/30$\mathregular{ms^{-1}}$', '1500m/30$\mathregular{ms^{-1}}$']
        props = dict(boxstyle='square', facecolor='white', alpha=1)
        ax.text(20, 115, sub_title[j-1], fontsize = 17, bbox = props, zorder = 6)
    
        if j == 1:
            #Title
            plt.title("Buoyancy [elapsed time = %d seconds]"  % secs, fontsize = 22, y = 1.19) 
    ...
    #Colorbar
    cbaxes = fig.add_axes([0.905, 0.2, 0.035, 0.55])             
    cbar = plt.colorbar(ref_plot, cax = cbaxes, ticks = levels_ticks)
    cbar.ax.tick_params(labelsize=11)
    plt.text(0.2, -0.14, '$\mathregular{ms^{-1}}$', fontsize = 19)
    
    
    path = '/uufs/chpc.utah.edu/common/home/u1013082/public_html/phd_plots/cm1/'
    plt.savefig('%s' % path + 'png_for_gifs/3panel_cross_section_30ms_buoyancy_%03d.png' % i, dpi = 100)
    plt.close(fig)
    

##Build GIF

os.system('convert -delay 12 -quality 100 /uufs/chpc.utah.edu/common/home/u1013082/public_html/phd_plots/cm1/png_for_gifs/3panel_cross_section_30ms_buoyancy_*.png /uufs/chpc.utah.edu/common/home/u1013082/public_html/phd_plots/cm1/gifs/3panel_cross_section_30ms_buoyancy.gif')

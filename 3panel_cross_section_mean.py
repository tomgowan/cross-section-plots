...
for j in range(1,4):
    ...
    #Levels for CREF
    levels_cref = np.arange(5,35.01,0.5)
    levels_cref_ticks = np.arange(5,35.01,5)
    
    #Plot reflectivity mean from 12000 to 24000 seconds (i = 100:200)
    ref_plot = plt.contourf(x2d, y2d, model_run.dbz[100:200,0:30,200,300:1200].mean(dim = 'time'), levels_cref, cmap = cmap_precip, extend = 'max', alpha = 1)
    ...
    if j == 1:
        #Title
        plt.title("Mean Reflectivity", fontsize = 22, y = 1.21) 
...
plt.savefig("/uufs/chpc.utah.edu/common/home/u1013082/public_html/phd_plots/cm1/plots/3panel_mtn_200m__cross_section_high_wind_mean_dbz.png", dpi = 500)
plt.close(fig)

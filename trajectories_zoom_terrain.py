...
for t in range(200,201):
    ...
    for run in range(1,7):
        ...
        #loop over runs
        run_name = ['mean_x_disp', 'mean_x_disph', 'mean_x_disp_small', 'mean_x_disp_smallh', 'mean_x_disp_tall', 'mean_x_disp_tallh']
        ...
        if run == 6:
            ax.text(0.55, -0.5, '1.5 hour time-mean', transform=ax.transAxes, bbox = props, fontsize = 16)
    ...
    #plt.text(0.2, -2.2, 'Time-mean change in ${\Theta}$ (K)', fontsize = 18)
    plt.text(0.12, -1.8, 'Time-mean x displacement (km)', fontsize = 18)
    
    #Quiver
#    plt.quiverkey(quiv, X=-1.65, Y=-0.2, U=20, linewidth = 0.75, color = 'k', label='20 $\mathregular{ms^{-1}}$', labelpos='W', fontproperties={'size': '28'})
    
    #Save and Close
    plt.savefig("/uufs/chpc.utah.edu/common/home/u1013082/public_html/phd_plots/cm1/plots/x_disp_terrain_xz.png", dpi = 150)
    plt.close(fig)  
